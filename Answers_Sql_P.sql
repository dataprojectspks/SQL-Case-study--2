...
IDManufacturer Int  IDENTITY(11, 1) PRIMARY KEY,
...
SELECT DISTINCT(DIM_LOCATION.State) State FROM FACT_TRANSACTIONS  
left join DIM_LOCATION on FACT_TRANSACTIONS.IDLocation = DIM_LOCATION.IDLocation
WHERE FACT_TRANSACTIONS.Date BETWEEN '2004-12-31' AND getdate()

--Q2: What state in the US is buying more 'Samsung' cell phones?

SELECT DISTINCT(DIM_LOCATION.State) State, sum(FACT_TRANSACTIONS.Quantity) qty
...
select FACT_TRANSACTIONS.idmodel as models,DIM_LOCATION.ZipCode as zipcodes, DIM_LOCATION.state as states, 
count(FACT_TRANSACTIONS.IDCustomer) as transactions
from FACT_TRANSACTIONS left join DIM_LOCATION
...
select top 1 dim_MODEL.Model_Name,  DIM_MANUFACTURER.Manufacturer_Name,DIM_MODEL.unit_price
from DIM_MODEL 
left join DIM_MANUFACTURER on DIM_MODEL.IDManufacturer = DIM_MANUFACTURER.IDManufacturer
order by DIM_MODEL.unit_price asc

--q5 Find out the average price for each model in the top5 
--manufacturers in terms of sales quantity and order by average price

select top 5 DIM_MANUFACTURER.Manufacturer_Name, cast(sum(totalprice)/sum(quantity) as int) 
as avg_price , DIM_MODEL.idmodel,sum(quantity) as total_qty from FACT_TRANSACTIONS
left join DIM_MODEL on FACT_TRANSACTIONS.IDModel=DIM_MODEL.idmodel 
left join DIM_MANUFACTURER on DIM_MANUFACTURER.IDManufacturer=DIM_MODEL.IDManufacturer
group by DIM_MANUFACTURER.Manufacturer_Name,DIM_MODEL.idmodel 
order by sum(quantity) desc ,avg_price

--q6:6.	List the names of the customers and the average amount spent in 2009, 
--where the average is higher than 500

select DIM_CUSTOMER.Customer_Name, avg(FACT_TRANSACTIONS.TotalPrice) av_price from FACT_TRANSACTIONS
left join DIM_CUSTOMER on FACT_TRANSACTIONS.idcustomer = DIM_CUSTOMER.IDCustomer
where FACT_TRANSACTIONS.Date between '2009-01-01' and '2009-12-31'
...
--q8: Show the manufacturer with the 2nd top sales in the year of 2009 
--and the manufacturer with the 2nd top sales in the year of 2010

select X.Manufacturer_name as Manufacturer_name_09,Y.Manufacturer_name as Manufacturer_name_10
 from (SELECT Manufacturer_name,row_number() over(order by SUM (TotalPrice)) as rn1
            ...
            group by Manufacturer_name)X inner join
    (SELECT Manufacturer_name,row_number() over(order by SUM (TotalPrice)) as rn2
            FROM Fact_Transactions FT
            LEFT JOIN DIM_Model DM ON FT.IDModel = DM.IDModel
            LEFT JOIN DIM_MANUFACTURER MFC  ON MFC.IDManufacturer = DM.IDManufacturer
            Where DATEPART(Year,date)=2010 
            group by Manufacturer_name)Y on X.rn1=Y.rn2  and rn1 in (1,2) and rn2 in (1,2)
...
SELECT TOP 100 
    YEAR(FACT_TRANSACTIONS.Date) year_trans,
    FACT_TRANSACTIONS.IDCustomer customerid,
    AVG(FACT_TRANSACTIONS.TotalPrice) avg_spend,
    AVG(FACT_TRANSACTIONS.Quantity) avg_qty
INTO DataSource
FROM 
    FACT_TRANSACTIONS
INNER JOIN 
...
GROUP BY 
    year(FACT_TRANSACTIONS.Date), FACT_TRANSACTIONS.IDCustomer
ORDER BY 
    AVG(FACT_TRANSACTIONS.TotalPrice) DESC 

SELECT *  ,
   CASE 
   when LAG(avg_spend, 1, 0) 
      OVER (PARTITION BY customerid
   ORDER BY year_trans, customerid)  IN (NULL,0) THEN NULL
   ELSE (LAG(avg_spend, 1, 0) 
      OVER (PARTITION BY customerid
   ORDER BY year_trans, customerid))
   END AS Lags_1
   into datasource_2
   from datasource 
   
select year_trans, customerid, 100*((avg_spend - Lags_1)/Lags_1) '%diff'
FROM DataSource_2
